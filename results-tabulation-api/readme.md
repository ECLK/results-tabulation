...
`set ENV_CONFIG=./env/dev.cfg && python manage.py create_database`

**Upgrade the Database**

`set ENV_CONFIG=./env/dev.cfg && python manage.py db upgrade`

**Run local server**

`set ENV_CONFIG=./env/dev.cfg && python index.py`
...
`set ENV_CONFIG=./env/test.cfg && pytest tests`
